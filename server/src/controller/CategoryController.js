...
class CategoryController {
  ...
  async store(req, res) {
    try {
      ...
      if (category) {
        return res
          .status(400)
          .json({ success: false, messages: "Category already exsist" });
      }
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (!category)
        return res.json({ success: false, messages: "Cant update Category" });
  ...
  async delete(req, res) {
    ...
    try {
      ...
      if (!category)
        return res
          .status(401)
          .json({ success: false, messages: "Cant delete Category" });
